...
- Stop report commit SHA: `7cd7eeec4b5d6290753a257fa0a6b8a5e7184eda`
...
- Added shared resolution order: `CodexCliRunnerOptions.codexBin`, then `CODEX_CLI_COMMAND`, then repo-managed `node_modules/.bin/codex`.
- `CodexCliRunner`, `ops:health`, and Codex OAuth login setup use the managed resolver.
- `scripts/bootstrap.sh` and `Dockerfile` assert `node_modules/.bin/codex` exists after `npm ci`.
- Docs now state global Codex CLI is not a fresh-install prerequisite.
- Codex auth remains separate; unattended live runner smoke still requires `OPENAI_API_KEY`.
...
`npm run live:smoke` was not run because `OPENCLAW_BASE_URL`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_TEST_CHAT_ID`, and `OPENAI_API_KEY` are absent in this shell.
