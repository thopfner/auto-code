...
- Vitest passed: 12 files, 43 tests.
...
Result: passed. Local health includes `api`, `web`, `worker`, `database`, `openclaw`, and `codex`; API/web are skipped locally until endpoint env vars are configured. Service log discovery returns local npm paths plus Docker Compose commands, and systemd journal commands for API and worker.

Docker Compose full rebuild and smoke:
...
Machine-readable artifact validation before QA clearance:

```bash
npx tsx -e '<validateForgeArtifacts requireCommitShas=true>'
```

Result: passed for current pushed artifacts before QA clearance. The status at that moment was still `QA_CHECKPOINT`, so `qaOutcome` was `unknown`, but required report, branch, SHA, and push checks passed.
