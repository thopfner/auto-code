...
Independent real runner smoke through `CodexCliRunner.run()`:

```bash
npx tsx -e '<read-only CodexCliRunner.run() smoke>'
```

Result: passed with installed `codex-cli 0.125.0`; the runner returned `AUTO_FORGE_QA_SMOKE_OK`.

Service-scoped runtime smoke:
...
Result: passed. API responded and worker started with runner `codex-cli`.
...
Result: passed for current pushed artifacts before QA clearance. The status at that moment was still `QA_CHECKPOINT`, so `qaOutcome` was `unknown`, but all required report, branch, SHA, and push checks passed.
