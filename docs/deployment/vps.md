...
The installer seeds OpenClaw Telegram channel config from the same Telegram bot token by writing `/root/.openclaw/.env` and `/root/.openclaw/telegram-bot-token` with mode `0600`, then setting `channels.telegram.tokenFile` to the token file. It also sets `channels.telegram.defaultTo` to the saved Telegram chat ID and, for private chats, allowlists the same positive numeric ID as the owner user ID. Live smoke reports routed Telegram delivery through `openclaw message send --channel telegram` as an optional diagnostic by default; controller Telegram replies use the Bot API directly. Set `AUTO_FORGE_REQUIRE_OPENCLAW_TELEGRAM_DELIVERY=1` when you intentionally want OpenClaw CLI Telegram delivery to fail the live gate.
...
Controller Telegram commands are authorized against the saved `TELEGRAM_TEST_CHAT_ID` by default. For private bot chats, the chat ID and user ID are normally the same positive number. If you intentionally use a group or a separate operator identity, set `TELEGRAM_OPERATOR_CHAT_ID` or `TELEGRAM_OPERATOR_USER_ID` in the runtime env before rerunning the installer.
